        ...
        if len(password) < self.min_length:
            raise ValidationError(
                ngettext(
                    ...
                    " password is too short. It must contain at least %(min_length)d character.",
                    # if it's more than one (which it probably is) put your message here
                    " პაროლის უნდა შეიცავდეს მინიმუმ %(min_length)d სიმბოლოს.",
                    self.min_length
                ),
...
class MyNumericPasswordValidator:
    ...
    def validate(self, password, user=None):
        if password.isdigit():
            raise ValidationError(
                ("არ შეიძლება რომ პაროლი შედგებოდეს მხოლოდ ციფრებით"),
